...
from src.input.input_files import file_to_matrix
...
def load_and_benchmark(input_directory, input_file_name, section_size):

    matrix, size, run = file_to_matrix(input_directory, input_file_name)

    print(f"Benchmark run {run} ... matrix size: {size}, section size: {section_size}")
    _, _, _, process_time, initial_rss, final_rss = check_and_benchmark(matrix, section_size)

    return size, run, section_size, process_time, initial_rss, final_rss
